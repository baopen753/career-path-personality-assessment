...
public class QuizQuestionService {
     ...
     */
    public List<QuizQuestionDTO> reorderQuestions(Long quizId, List<Long> questionIds) {
        ...
        for (int i = 0; i < questionIds.size(); i++) {
            ...
            QuizQuestion question = quizQuestionRepository.findById(questionId)
                    .orElseThrow(() -> new QuizNotFoundException("Quiz question not found with id: " + questionId));
    ...
    /**
     * Duplicate a question within the same quiz or to another quiz
     */
    public QuizQuestionDTO duplicateQuestion(Long questionId, Long targetQuizId) {
        log.info("Duplicating question ID: {} to quiz ID: {}", questionId, targetQuizId);

        QuizQuestion originalQuestion = quizQuestionRepository.findById(questionId)
                .orElseThrow(() -> new QuizNotFoundException("Quiz question not found with id: " + questionId));

        // Verify target quiz exists
        if (!quizRepository.existsById(targetQuizId)) {
            throw new QuizNotFoundException("Target quiz not found with id: " + targetQuizId);
        }

        // Create new question
        QuizQuestion newQuestion = new QuizQuestion();
        newQuestion.setContent(originalQuestion.getContent() + " (Copy)");
        newQuestion.setDimension(originalQuestion.getDimension());
        newQuestion.setQuizId(targetQuizId);
        newQuestion.setOrderNumber(getMaxOrderNumberForQuiz(targetQuizId) + 1);

        QuizQuestion savedQuestion = quizQuestionRepository.save(newQuestion);

        // Duplicate options
        List<QuizOptions> originalOptions = quizOptionsRepository.findByQuestionId(questionId);
        List<QuizOptions> newOptions = originalOptions.stream()
                .map(option -> {
                    QuizOptions newOption = new QuizOptions();
                    newOption.setOptionText(option.getOptionText());
                    newOption.setTargetTrait(option.getTargetTrait());
                    newOption.setScoreValue(option.getScoreValue());
        ...
        log.info("Question duplicated successfully with new ID: {}", savedQuestion.getId());
        return convertToDTO(savedQuestion);
    }
     ...
     */
    @Transactional(readOnly = true)
    public QuestionStatisticsDTO getQuestionStatistics(Long quizId) {
        ...
        // Count by dimension
        stats.setDimensionDistribution(questions.stream()
                .collect(Collectors.groupingBy(
                        QuizQuestion::getDimension,
                        Collectors.counting())));
